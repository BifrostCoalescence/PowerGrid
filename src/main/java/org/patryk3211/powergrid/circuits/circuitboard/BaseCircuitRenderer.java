...
public class BaseCircuitRenderer<B extends BaseCircuitBlock, E extends BaseCircuitBE> extends SafeBlockEntityRenderer<E> {
    protected float offsetY = 2;

    public BaseCircuitRenderer(BlockEntityRendererProvider.Context context) {
    }
    ...
    protected void renderSafe(CircuitBoardBlockEntity be, float partialTicks, PoseStack ms, MultiBufferSource bufferSource, int light, int overlay) {
        ...
        for(var placed : components) {
            ...
            stack.pushPose()
                    .center()
                    .rotateYDegrees(BaseCircuitBlock.getAngleY(state))
                    .rotateXDegrees(BaseCircuitBlock.getAngleX(state))
                    .uncenter()
                    .translate(placed.x / 16f, offsetY / 16f, placed.y / 16f);
